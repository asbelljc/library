...
if (!storageAvailable('localStorage') || !localStorage.getItem('myLibrary')) {
  ...
  let book2 = new Book("Oh, the Places You'll Go!", 'Dr. Seuss', 64, true);
  let book3 = new Book('A Brief History of Time', 'Stephen Hawking', 256, true);
...
function sortBooks() {
  myLibrary.sort((a, b) => {
    ...
    if (getOption()[0] === 'title') {
      return (
        a.title.localeCompare(b.title) || aLastName.localeCompare(bLastName)
      );
    } else {
      return (
        aLastName.localeCompare(bLastName) || a.title.localeCompare(b.title)
      );
    }
  });
...
function makeForm() {
  ...
  const submitBtnSvg = document.createElementNS(
    'http://www.w3.org/2000/svg',
    'svg'
  );
  const submitBtnPath = document.createElementNS(
    'http://www.w3.org/2000/svg',
    'path'
  );
  const cancelButton = document.createElement('button');
  const cancelBtnSvg = document.createElementNS(
    'http://www.w3.org/2000/svg',
    'svg'
  );
  const cancelBtnPath = document.createElementNS(
    'http://www.w3.org/2000/svg',
    'path'
  );
...
function populateGrid() {
  ...
  myLibrary.forEach((book) => {
    ...
    const delBtnSvg = document.createElementNS(
      'http://www.w3.org/2000/svg',
      'svg'
    );
    const delBtnPath = document.createElementNS(
      'http://www.w3.org/2000/svg',
      'path'
    );
